...
-- 添加停止按钮
if is_running("mentohust") == translate("RUNNING") then
    stop = s:option(Button, "_stop", translate("Stop MentoHUST"))
    stop.inputtitle = translate("Stop")
    stop.inputstyle = "reset"
    function stop.write()
        luci.http.redirect(luci.dispatcher.build_url("admin", "services", "mentohust", "stop"))
    end
end
